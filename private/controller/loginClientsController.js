...
export const registerClientAppHandler = async (req, res) => {
    try {
        ...
        await sendEmail({
            destino: correo, asunto: "Lavandería MARVI- ¡Registro completo!", html: htmlContent,
        });
...
export const loginClientHandler = async (req, res) => {
    try {
        ...
        if (error.message === "El cliente no existe") res.status(500).json({message: "Error de autenticación, Las credenciales son invalidas"}); else res.status(500).json({message: `Error al iniciar sesión, ${error.message}`});
    }
};
...
export const resetPasswordClientEmailHandler = async (req, res) => {
    try {
        ...
        await sendEmail({
            destino: correo, asunto: "Lavandería MARVI - Restablece tu contraseña", html: htmlContent,
        });

        res.status(200).json({message: "Correo enviado, Sigue las instrucciones para restablecer tu contraseña"});
    } catch (error) {
        if (error.message === "El correo electrónico no existe") res.status(500).json({message: "Error al enviar correo, El correo electrónico no está registrado"}); else res.status(500).json({message: `Error al enviar correo, ${error.message}`});
    }
};
...
export const resetPasswordClientHandler = async (req, res) => {
    try {
        ...
        const template = fs.readFileSync("./templates/message_email.html", "utf8");
        const htmlContent = template.replace("_asunto", "¡Restablecimiento de contraseña exitoso!").replace("_mensaje", "Tu contraseña ha sido restablecida exitosamente, ahora puedes iniciar sesión con tu nueva contraseña.");

        await sendEmail({
            destino: correo, asunto: "Lavandería MARVI- Restablecimiento de contraseña", html: htmlContent,
        });

        res.status(200).json({message: response.rows[0]});
    } catch (error) {
        if (error.message === "El cliente no existe") res.status(500).json({message: "Error al restablecer la contraseña, El correo electrónico no está registrado"}); else res.status(500).json({message: `Error al restablecer la contraseña, ${error.message}`});
    }
};
