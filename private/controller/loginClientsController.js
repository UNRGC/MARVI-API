...
export const getClientCodeHandler = async (req, res) => {
    try {
        ...
        if (error.message === "El código no existe") res.status(500).json({message: "Error al verificar el código, el código no está registrado."}); else res.status(500).json({message: `Error al verificar el código, ${error.message}`});
    }
}
...
export const resetPasswordClientEmailHandler = async (req, res) => {
    try {
        ...
        if (error.message === "El correo electrónico no existe") res.status(500).json({message: "Error al restablecer la contraseña, el correo electrónico no está registrado."}); else res.status(500).json({message: `Error al restablecer la contraseña, ${error.message}`});
    }
};
...
export const resetPasswordClientHandler = async (req, res) => {
    try {
        ...
        if (error.message === "El cliente no existe") res.status(500).json({message: "Error al restablecer la contraseña, el correo no está registrado."}); else res.status(500).json({message: `Error al restablecer la contraseña, ${error.message}`});
    }
};
