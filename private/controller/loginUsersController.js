...
import {changeState, closeAllSessions, forgotPassword, login} from "../model/loginUsersModel.js";
import {getUser} from "../model/usersModel.js";
...
export const loginHandler = async (req, res) => {
    try {
        ...
        userResponse.rows[0].fecha_registro = dateTZFechaRegistro.format(process.env.DATE_FORMAT);

        const dateTZUltimaActividad = moment.utc(userResponse.rows[0].ultima_actividad).tz(process.env.TIME_ZONE);
        userResponse.rows[0].ultima_actividad = dateTZUltimaActividad.format(`${process.env.DATE_FORMAT} ${process.env.TIME_FORMAT}`);
...
export const forgotPasswordHandler = async (req, res) => {
    try {
        ...
        const template = fs.readFileSync("templates/recovery_password_user.html", "utf8");
