...
export const getProductHandler = async (req, res) => {
    try {
        ...
        if (response.rows[0].ultima_compra) {
            ...
            response.rows[0].ultima_compra = dateTZUltimaCompra.format(process.env.DATE_FORMAT);
        }
        ...
        if (response.rows[0].proxima_compra) {
            ...
            response.rows[0].proxima_compra = dateTZProximaCompra.format(process.env.DATE_FORMAT);
        }
    ...
    try {
        ...
        response.rows.forEach((producto) => {
            if (producto.ultima_compra) {
                ...
                producto.ultima_compra = dateTZUltimaCompra.format(process.env.DATE_FORMAT);
            }

            if (producto.proxima_compra) {
                const dateTZProximaCompra = moment.utc(producto.proxima_compra).tz(process.env.TIME_ZONE);
                producto.proxima_compra = dateTZProximaCompra.format(process.env.DATE_FORMAT);
            }
        });
