...
ws.onmessage = (event) => {
    ...
    if (event.data.includes("Error:")) messageElement.classList.add("error");
    else if (event.data.includes("Conectado") || event.data.includes("éxito")) messageElement.classList.add("success");
    else if (event.data.includes("Creando")) messageElement.classList.add("warning");
...
formConnection.addEventListener("submit", async (e) => {
    ...
    if (await alertConfirm("¿Deseas aplicar los cambios?", "Esta acción no se puede deshacer. Asegúrate de tener las credenciales correctas antes de proceder.", "warning")) {
        const response = await updateConnection(userInput.value, passInput.value, serverInput.value, databaseInput.value, serviceSelect.value, emailInput.value, passwordInput.value, timeZoneInput.value, dateFormatInput.value);
