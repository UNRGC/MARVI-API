...
export const getClientByEmail = async (correo) => {
    ...
    const response = await client.query("SELECT * FROM consultar_correo_cliente($1);", [correo]);
...
export const getClient = async (codigo) => {
    // Obtener un cliente de la base de datos
    const response = await client.query("SELECT * FROM consultar_cliente($1);", [codigo]);
...
export const getClients = async (data) => {
    // Obtener un cliente de la base de datos
    const response = await client.query("SELECT * FROM filtrar_clientes($1, $2, $3::INT, $4::INT);", [data.columna_orden, data.orden, data.limit, data.offset]);
...
export const searchClients = async (data) => {
    ...
    const response = await client.query("SELECT * FROM buscar_clientes($1, $2::INT, $3::INT)", [data.busqueda, data.limit, data.offset]);
