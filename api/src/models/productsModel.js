...
export const getProduct = async (codigo) => {
    ...
    const response = await client.query("SELECT * FROM consultar_producto($1);", [codigo]);
...
export const getProducts = async (data) => {
    ...
    const response = await client.query("SELECT * FROM filtrar_productos($1, $2, $3::INT, $4::INT);", [data.columna_orden, data.orden, data.limit, data.offset]);
...
export const searchProducts = async (data) => {
    // Obtener los productos de la base de datos
    const response = await client.query("SELECT * FROM buscar_productos($1, $2::INT, $3::INT);", [data.busqueda, data.limit, data.offset]);

    // Retornar
    return response;
};

// Función para crear la proxima compra
